...
class ChapterWorker:
    ...
    async def process_chapter(
        ...
        try:
            ...
            logger.info(f"Worker {self.worker_id}: Tamanho do conteúdo: {len(content):,} caracteres")
            ...
            if not self.progress_manager.start_chapter(chapter_id):
                logger.warning(f"Worker {self.worker_id}: Capítulo {chapter_id} já foi iniciado")
                return False
            ...
            logger.info(f"Worker {self.worker_id}: Fragmentação concluída - {len(chunks) if chunks else 0} chunks gerados")
            if not chunks:
                logger.warning(f"Worker {self.worker_id}: Capítulo {chapter_id} não gerou chunks")
            ...
            # Traduz cada chunk
            logger.info(f"Worker {self.worker_id}: Iniciando tradução de {len(chunks)} chunks...")
            translated_chunks = []
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    logger.debug(f"Worker {self.worker_id}: Traduzindo chunk {i+1}/{len(chunks)} ({chunk_size} chars)")
                    
                    # Rate limiting
                    if self.rate_limiter:
                        logger.debug(f"Worker {self.worker_id}: Aplicando rate limit...")
                        await self.rate_limiter.acquire()
                    
                    # Traduz o chunk
                    logger.debug(f"Worker {self.worker_id}: Enviando chunk para tradução...")
                    translated_text = await self.translator.translate_text(chunk.content)
                    ...
                    if progress_callback:
                        progress_callback(self.worker_id, chapter_id, i + 1, len(chunks))
                    
                    chunk_time = time.time() - chunk_start_time
                    logger.info(f"Worker {self.worker_id}: Chunk {i+1}/{len(chunks)} traduzido em {chunk_time:.1f}s ({chunk_size} → {len(translated_text)} chars)")
                    
                except Exception as e:
                    chunk_time = time.time() - chunk_start_time
                    logger.error(f"Worker {self.worker_id}: ERRO ao traduzir chunk {i+1}/{len(chunks)}: {e}")
                    logger.error(f"Worker {self.worker_id}: Chunk falhado após {chunk_time:.1f}s, tamanho: {chunk_size} chars")
                    logger.debug(f"Worker {self.worker_id}: Conteúdo do chunk com erro: {chunk.content[:200]}{'...' if len(chunk.content) > 200 else ''}")
                    
                    self.stats.errors_count += 1
                    # Em caso de erro, mantém o texto original
                    translated_chunks.append(chunk.content)
                    logger.warning(f"Worker {self.worker_id}: Usando texto original para chunk {i+1}")
            
            # Junta todos os chunks traduzidos
            logger.debug(f"Worker {self.worker_id}: Juntando {len(translated_chunks)} chunks traduzidos...")
            full_translated_content = '\n\n'.join(translated_chunks)
            final_size = len(full_translated_content)
            original_size = len(content)
            size_change = ((final_size - original_size) / original_size * 100) if original_size > 0 else 0
            
            logger.info(f"Worker {self.worker_id}: Conteúdo final: {original_size:,} → {final_size:,} chars ({size_change:+.1f}%)")
            ...
            logger.debug(f"Worker {self.worker_id}: Marcando capítulo como completado...")
            self.progress_manager.complete_chapter(chapter_id)
            ...
            chars_per_sec = original_size / processing_time if processing_time > 0 else 0
            ...
            logger.info(f"Worker {self.worker_id}: Velocidade: {chunks_per_sec:.1f} chunks/s, {chars_per_sec:.0f} chars/s")
            logger.info(f"Worker {self.worker_id}: Erros neste capítulo: {self.stats.errors_count}")
            ...
            logger.error(f"Worker {self.worker_id}: Tempo até erro: {processing_time:.1f}s")
            logger.error(f"Worker {self.worker_id}: Tipo do erro: {type(e).__name__}")
            logger.error(f"Worker {self.worker_id}: Mensagem: {str(e)}")
            logger.error(f"Worker {self.worker_id}: Tamanho do conteúdo: {len(content):,} chars")
...
class ParallelProcessor:
    ...
    async def create_workers(self, api_key: Optional[str] = None) -> None:
        """
    ...
    async def process_chapters(
        ...
        """
        if not self.workers:
            raise RuntimeError("Workers não foram criados. Chame create_workers() primeiro.")
        ...
        if resume:
            ...
            logger.info(f"Retomando processamento: {len(chapters_to_process)} capítulos pendentes")
        else:
            chapters_to_process = chapters
            logger.info(f"Iniciando processamento: {len(chapters_to_process)} capítulos")
        ...
        async def process_with_semaphore(worker: ChapterWorker, chapter: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Processamento concluído: {successes} sucessos, {failures} falhas em {processing_time:.1f}s")
    ...
    def _get_processing_stats(self, total_time: float) -> Dict[str, Any]:
        ...
        total_chapters = sum(stats.chapters_processed for stats in self.worker_stats.values())
        total_chunks = sum(stats.chunks_processed for stats in self.worker_stats.values())
