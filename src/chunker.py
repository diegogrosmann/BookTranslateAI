...
class TextChunker:
    ...
    def chunk_text(self, text: str, chapter_id: str) -> list[TextChunk]:
        ...
        logger.debug(f"Preserve sentences: {self.preserve_sentences}")
        logger.debug(f"Preserve paragraphs: {self.preserve_paragraphs}")

        if not text.strip():
            logger.warning("Empty text or only spaces - returning empty list")
            return []
        ...
        logger.debug(f"Estimated chunks: {estimated_chunks}")

        while current_pos < len(text):
            logger.debug(f"--- Processing chunk {chunk_id + 1} ---")
            logger.debug(f"Current position: {current_pos:,}/{len(text):,}")
            ...
            if chunk_end < len(text):
                logger.debug(f"Looking for natural break near position {chunk_end}")
                chunk_end = self._find_natural_break(text, current_pos, chunk_end)

                if chunk_end != original_chunk_end:
                    natural_breaks_found += 1
                    logger.debug(
                        f"Natural break found: {original_chunk_end} → {chunk_end}"
                    )
                else:
                    forced_breaks += 1
                    logger.debug(f"Forced break at position {chunk_end}")
            ...
            logger.debug(
                f"Chunk {chunk_id + 1}: {current_pos}-{chunk_end} ({chunk_size_actual:,} chars)"
            )
            ...
            if next_pos <= current_pos:
                ...
                logger.debug(
                    f"Ajustando posição para evitar loop: {current_pos} → {next_pos}"
                )
        ...
        logger.info("=== FRAGMENTAÇÃO CONCLUÍDA ===")
    ...
    def _find_natural_break(self, text: str, start_pos: int, target_end: int) -> int:
        ...
        logger.debug(
            f"Buscando quebra natural: janela {search_start}-{search_end} (alvo: {target_end})"
        )
        ...
        if self.preserve_paragraphs:
            ...
            if paragraph_breaks:
                ...
                logger.debug(
                    f"Quebra de parágrafo selecionada na posição {final_pos} (distância: {abs(final_pos - target_end)})"
                )
                return final_pos
        ...
        if self.preserve_sentences:
            ...
            if sentence_breaks:
                ...
                final_pos = search_start + best_break.end()
                logger.debug(
                    f"Quebra de sentença selecionada na posição {final_pos} (distância: {abs(final_pos - target_end)})"
                )
                return final_pos

        # Se não encontrou quebra natural, usa a posição original
        logger.debug(
            f"Nenhuma quebra natural encontrada, usando posição original: {target_end}"
        )
        return target_end
    ...
    def chunk_chapters(self, chapters: list[dict[str, str]]) -> list[list[TextChunk]]:
        ...
        logger.info(
            f"Total de {total_chunks} chunks criados para {len(chapters)} capítulos"
        )
    ...
    def adjust_chunk_size_for_model(
        self, model_name: str, max_tokens: int | None = None
    ) -> None:
        ...
        new_chunk_size = int(
            available_tokens * chars_per_token * 0.8
        )  # 80% do disponível

        if new_chunk_size != self.chunk_size:
            logger.info(
                f"Ajustando chunk_size de {self.chunk_size} para {new_chunk_size} para modelo {model_name}"
            )
