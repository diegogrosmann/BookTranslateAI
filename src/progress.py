...
try:
    from src.document_generator import generate_documents

    DOCUMENT_GENERATOR_AVAILABLE = True
except ImportError:
    DOCUMENT_GENERATOR_AVAILABLE = False
    logging.warning(
        "document_generator não disponível. Geração de EPUB/PDF desabilitada."
    )
...
class ProgressManager:
    ...
    def __init__(self, progress_file: str):
        ...
        self.progress: TranslationProgress | None = None
        ...
        self._save_debounce_interval = (
            2.0  # Salva no máximo a cada 2 segundos para chunks
        )
        ...
        while not self._stop_saving.is_set():
            try:
                ...
                if save_type == "important" or self._pending_important_save:
                    ...
                    if (
                        time_since_last_save < 0.5
                    ):  # Aguarda menos para operações importantes
    ...
    def load_progress(self) -> TranslationProgress | None:
        """
        ...
        try:
            ...
            for chapter_id, chapter_data in data.get("chapters", {}).items():
                ...
                if "original_index" not in chapter_data:
                    ...
                    logger.debug(
                        f"Adicionado original_index=0 para compatibilidade: {chapter_id}"
                    )
            ...
            logger.info(
                f"Progresso carregado: {progress.completed_chapters}/{progress.total_chapters} capítulos"
            )
            return progress
    ...
    def complete_chapter(self, chapter_id: str) -> None:
        ...
        with self._lock:
            ...
            self.progress.completed_chapters = sum(
                1 for ch in self.progress.chapters.values() if ch.status == "completed"
            )
    ...
    def get_pending_chapters(self) -> list[str]:
        ...
        return [
            chapter_id
            for chapter_id, chapter in self.progress.chapters.items()
    ...
    def can_resume(self, input_file: str, model: str, target_language: str) -> bool:
        ...
        return (
            self.progress.input_file == input_file
            and self.progress.model == model
            and self.progress.target_language == target_language
            and self.progress.status in ["in_progress", "paused"]
        )
...
class OutputManager:
    ...
    def __init__(
        ...
        book_title: str = "Livro Traduzido",
        book_author: str = "Autor Desconhecido",
    ):
        """
...
"""
    ...
    def append_chapter(
        self, chapter_title: str, translated_content: str, chapter_id: str | None = None
    ) -> None:
        """
...
"""
                            f.write(chapter_md)
                            logger.debug(
                                f"Capítulo adicionado ao MD: {formatted_title} (índice original: {original_index})"
                            )
                        else:
                            logger.warning(
                                f"Capítulo sem conteúdo traduzido: {chapter.title}"
                            )
    ...
    def _create_smart_ordering_key(self, chapter_data) -> tuple:
        ...
        if chapter_id.startswith("part"):
            chapter_group, chapter_order, section_type = self._decode_part_number(
                chapter_id
            )
    ...
    def _sort_chapters_smartly(self, chapters_dict) -> list:
        ...
        for chapter_id, chapter_data in chapters_dict.items():
            ...
            chapters_list.append(
                (smart_key, chapter_data.original_index, chapter_id, chapter_data)
            )
        ...
        return [
            (original_index, chapter_id, chapter_data)
    ...
    def consolidate_chapters_if_needed(self) -> bool:
        ...
        """
        logger.warning(
            "consolidate_chapters_if_needed() está obsoleto. Use generate_final_documents_from_progress()"
        )
        return True
    ...
    def _generate_final_documents(self) -> None:
        ...
        try:
            if not os.path.exists(self.output_file):
                logger.warning(
                    "Arquivo de saída não existe, pulando geração de documentos"
                )
                return
