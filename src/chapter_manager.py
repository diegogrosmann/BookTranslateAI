...
class ChapterFileManager:
    ...
    def register_chapter(self, chapter_number: int, chapter_id: str, title: str) -> ChapterFile:
        """
        ...
        with self._lock:
            ...
            logger.debug(f"Capítulo registrado: {chapter_number} - {title} -> {filename}")
    ...
    def consolidate_chapters(self, final_output_file: str) -> bool:
        ...
        try:
            ...
            with open(final_output_file, 'w', encoding='utf-8') as output_file:
                ...
                output_file.write(f"""# {self.book_title}
...
""")
                ...
                for chapter_file in completed_chapters:
                    ...
                    try:
                        with open(chapter_file.filepath, 'r', encoding='utf-8') as chapter_f:
...
""")
                        ...
                        logger.error(f"Erro ao ler capítulo {chapter_file.filename}: {e}")
                        continue
            
            logger.info(f"Consolidação concluída: {len(completed_chapters)} capítulos → {final_output_file}")
            return True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            completed_chapters = sum(1 for cf in self.chapter_files.values() if cf.completed)
