    ...
    def generate(
        ...
        for i, (chapter_title, chapter_content) in enumerate(chapters, 1):
            ...
            chapter = epub.EpubHtml(
                title=chapter_title, file_name=f"chapter_{i:03d}.xhtml", lang=language
            )
        ...
        # Cria índice
        book.toc = [
            (epub.Link(f"chapter_{i:03d}.xhtml", title, f"chapter_{i:03d}"))
        ...
        # Salva o arquivo
        output_path = (
            self.output_dir / f"{self._sanitize_filename(self.base_filename)}.epub"
        )
        ...
        for line in lines:
            ...
            for pattern in chapter_patterns:
                ...
                if match:
                    ...
                    if current_chapter_content and any(
                        c.strip() for c in current_chapter_content
                    ):
                        chapters.append(
                            (
            ...
            if not is_chapter_start:
                ...
                if (
                    found_first_chapter or line
                ):  # Só adiciona se já encontrou um capítulo ou não é linha vazia
...
class PdfGenerator(DocumentGenerator):
    ...
    def generate(
        ...
        output_path = (
            self.output_dir / f"{self._sanitize_filename(self.base_filename)}.pdf"
        )
        ...
        story.append(
            Paragraph(
                f"Traduzido em {datetime.now().strftime('%d/%m/%Y')}", styles["Date"]
            )
        )
