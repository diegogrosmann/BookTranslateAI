...
def setup_logging(
    ...
    terminal_handler: CleanTerminalHandler | logging.StreamHandler
    if clean_terminal:
    ...
    litellm_level = (
        logging.WARNING if numeric_level >= logging.WARNING else logging.INFO
    )
    ...
    logger.info(
        f"Logging configurado - Nível: {log_level}, Arquivo: {log_file or 'Nenhum'}"
    )
...
class ProgressLogger:
    ...
    def log_start(
        self, input_file: str, output_file: str, model: str, total_chapters: int
    ):
    ...
    def log_chapter_complete(
        self, chapter_title: str, worker_id: int, time_taken: float
    ):
    ...
    def log_chunk_progress(self, chapter_id: str, completed: int, total: int):
        ...
        self.logger.debug(
            f"Capítulo {chapter_id}: {completed}/{total} chunks ({percentage:.1f}%)"
        )
    ...
    def log_completion(
        self, total_time: float, chapters_processed: int, chunks_processed: int
    ):
