...
from src.logging_config import get_log_file_path, progress_logger, setup_logging
...
def load_context_from_file(context_file: str) -> str:
    ...
    try:
        ...
        console.print(
            f"✓ Contexto carregado de {context_file} ({len(content)} caracteres)",
            style="green",
        )
        return content
    except Exception as e:
        console.print(
            f"✗ Erro ao carregar contexto de {context_file}: {e}", style="red"
        )
        return ""
...
class ProgressDisplay:
    ...
    def update_chapter_progress(
        self, worker_id: int, chapter_id: str, completed_chunks: int, total_chunks: int
    ):
        ...
        if completed_chunks >= total_chunks:
            self.current_progress.update(
                task_id, description=f"[green]✓ {chapter_desc}"
            )
...
@click.option(
    "--context", default="", help="Contexto adicional para orientar a tradução"
)
@click.option(
    "--context-file",
    type=click.Path(exists=True),
...
@click.option(
    "--max-workers", default=4, type=int, help="Número máximo de workers paralelos"
)
@click.option(
    "--rate-limit",
    default=2.0,
...
@click.option(
    "--resume/--no-resume", default=True, help="Retomar tradução existente se possível"
)
@click.option(
    "--log-level",
    default="INFO",
...
@click.option(
    "--clean-terminal/--verbose-terminal",
    default=True,
    help="Terminal limpo (apenas progresso) vs. verbose",
)
@click.option(
    "--test-connection", is_flag=True, help="Apenas testa conexão com o modelo e sai"
)
@click.option("--list-models", is_flag=True, help="Lista modelos disponíveis e sai")
...
def main(
    ...
    setup_logging(log_level=log_level, log_file=log_file, clean_terminal=clean_terminal)
    ...
    translation_config = TranslationConfig(
        model=model, target_language=target_lang, context=context
    )
...
[bold]Idioma Alvo:[/bold] {info["target_language"]}"""
    ...
    if not api_key:
        console.print(
            "✗ [red]Chave da API é obrigatória. Use --api-key ou defina API_KEY[/red]"
        )
        sys.exit(1)
...
async def translate_book(
    ...
    try:
        ...
        book_author = (
            "Autor Desconhecido"  # Poderia ser extraído dos metadados se disponível
        )
        ...
        console.print(
            "📚 [green]Documentos MD, EPUB e PDF serão gerados no final[/green]"
        )
        ...
        if resume:
            ...
            if existing_progress:
                logger.info(
                    f"Progresso encontrado: {len(existing_progress.chapters)} capítulos"
                )
                ...
                if can_resume:
                    ...
                    console.print("📋 [green]Retomando tradução existente...[/green]")
                    logger.info(
                        f"Retomando tradução - {len(pending_chapters)} capítulos pendentes"
                    )
                    progress_logger.log_resume(len(pending_chapters))
                else:
                    existing_progress = None
                    logger.warning(
                        "Progresso incompatível - parâmetros diferentes detectados"
                    )
                    console.print(
                        "⚠ [yellow]Não foi possível retomar - iniciando nova tradução[/yellow]"
                    )
            else:
        ...
        if not existing_progress:
            ...
            try:
                extractor = ContentExtractorFactory.create_extractor(
                    input_file, format_override
                )
                logger.info(f"Extrator criado: {type(extractor).__name__}")

                chapters = extractor.extract_content(input_file)
                logger.info(
                    f"Extração concluída - {len(chapters) if chapters else 0} capítulos encontrados"
                )
                ...
                if chapters:
                    ...
                    for i, chapter in enumerate(chapters):
                        ...
                        logger.debug(
                            f"Capítulo {i + 1}: '{title}' - {content_length:,} caracteres"
                        )
            ...
            console.print(
                f"✓ [green]Extraídos {len(chapters)} capítulos/seções[/green]"
            )
            ...
            console.print("📋 [cyan]Carregando capítulos do progresso salvo...[/cyan]")
        ...
        if old_chunk_size != new_chunk_size:
            logger.info(
                f"Chunk size ajustado para modelo {translation_config.model}: {old_chunk_size} → {new_chunk_size}"
            )
        else:
        ...
        def progress_callback(
            worker_id: int, chapter_id: str, completed_chunks: int, total_chunks: int
        ):
            progress_display.update_chapter_progress(
                worker_id, chapter_id, completed_chunks, total_chunks
            )
        ...
        progress_logger.log_start(
            input_file, output_file, translation_config.model, total_chapters
        )
        ...
        try:
            ...
            success = output_manager.generate_final_documents_from_progress(
                progress_manager
            )
            if success:
                console.print("✓ [green]Arquivos finais gerados com sucesso[/green]")
                ...
                console.print(
                    "⚠ [yellow]Erro durante geração dos arquivos finais[/yellow]"
                )
            ...
            logger.info(
                f"Tempo médio por capítulo: {stats['average_time_per_chapter']:.1f}s"
            )
...
[bold]Tempo Médio por Capítulo:[/bold] {stats["average_time_per_chapter"]:.1f}s"""
            ...
            console.print(
                Panel(stats_content, title="Estatísticas", border_style="green")
            )
            ...
            console.print("🔄 [cyan]Use --resume para continuar[/cyan]")
